...
namespace validation_layer
{
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetModuleGetDebugInfo(hModule, format, pSize, pDebugInfo)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDeviceGetDebugProperties(hDevice, pDebugProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugAttach(hDevice, config, phDebug)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugDetach(hDebug)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugReadEvent(hDebug, timeout, event)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugAcknowledgeEvent(hDebug, event)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugInterrupt(hDebug, thread)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugResume(hDebug, thread)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugReadMemory(hDebug, thread, desc, size, buffer)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugWriteMemory(hDebug, thread, desc, size, buffer)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugGetRegisterSetProperties(hDevice, pCount, pRegisterSetProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugGetThreadRegisterSetProperties(hDebug, thread, pCount, pRegisterSetProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugReadRegisters(hDebug, thread, type, start, count, pRegisterValues)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDebugWriteRegisters(hDebug, thread, type, start, count, pRegisterValues)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupGet(hDevice, pCount, phMetricGroups)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupGetProperties(hMetricGroup, pProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupCalculateMetricValues(hMetricGroup, type, rawDataSize, pRawData, pMetricValueCount, pMetricValues)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGet(hMetricGroup, pCount, phMetrics)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGetProperties(hMetric, pProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetContextActivateMetricGroups(hContext, hDevice, count, phMetricGroupsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricStreamerOpen(hContext, hDevice, hMetricGroup, desc, hNotificationEvent, phMetricStreamer)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetCommandListAppendMetricStreamerMarker(hCommandList, hMetricStreamer, value)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricStreamerClose(hMetricStreamer)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricStreamerReadData(hMetricStreamer, maxReportCount, pRawDataSize, pRawData)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricQueryPoolCreate(hContext, hDevice, hMetricGroup, desc, phMetricQueryPool)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricQueryPoolDestroy(hMetricQueryPool)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricQueryCreate(hMetricQueryPool, index, phMetricQuery)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricQueryDestroy(hMetricQuery)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricQueryReset(hMetricQuery)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetCommandListAppendMetricQueryBegin(hCommandList, hMetricQuery)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetCommandListAppendMetricQueryEnd(hCommandList, hMetricQuery, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetCommandListAppendMetricMemoryBarrier(hCommandList)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricQueryGetData(hMetricQuery, pRawDataSize, pRawData)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetKernelGetProfileInfo(hKernel, pProfileProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetTracerExpCreate(hContext, desc, phTracer)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetTracerExpDestroy(hTracer)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetTracerExpSetPrologues(hTracer, pCoreCbs)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetTracerExpSetEpilogues(hTracer, pCoreCbs)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetTracerExpSetEnabled(hTracer, enable)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDeviceGetConcurrentMetricGroupsExp(hDevice, metricGroupCount, phMetricGroups, pMetricGroupsCountPerConcurrentGroup, pConcurrentGroupCount)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricTracerCreateExp(hContext, hDevice, metricGroupCount, phMetricGroupsLocal, desc, hNotificationEvent, phMetricTracer)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricTracerDestroyExp(hMetricTracer)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricTracerEnableExp(hMetricTracer, synchronous)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricTracerDisableExp(hMetricTracer, synchronous)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricTracerReadDataExp(hMetricTracer, pRawDataSize, pRawData)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricDecoderCreateExp(hMetricTracer, phMetricDecoder)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricDecoderDestroyExp(phMetricDecoder)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricDecoderGetDecodableMetricsExp(hMetricDecoder, pCount, phMetrics)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricTracerDecodeExp(phMetricDecoder, pRawDataSize, pRawData, metricsCount, phMetricsLocal, pSetCount, pMetricEntriesCountPerSet, pMetricEntriesCount, pMetricEntries)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupCalculateMultipleMetricValuesExp(hMetricGroup, type, rawDataSize, pRawData, pSetCount, pTotalMetricValueCount, pMetricCounts, pMetricValues)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupGetGlobalTimestampsExp(hMetricGroup, synchronizedWithHost, globalTimestamp, metricTimestamp)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupGetExportDataExp(hMetricGroup, pRawData, rawDataSize, pExportDataSize, pExportData)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupCalculateMetricExportDataExp(hDriver, type, exportDataSize, pExportData, pCalculateDescriptor, pSetCount, pTotalMetricValueCount, pMetricCounts, pMetricValues)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricProgrammableGetExp(hDevice, pCount, phMetricProgrammables)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricProgrammableGetPropertiesExp(hMetricProgrammable, pProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricProgrammableGetParamInfoExp(hMetricProgrammable, pParameterCount, pParameterInfo)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricProgrammableGetParamValueInfoExp(hMetricProgrammable, parameterOrdinal, pValueInfoCount, pValueInfo)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricCreateFromProgrammableExp2(hMetricProgrammable, parameterCount, pParameterValues, pName, pDescription, pMetricHandleCount, phMetricHandles)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricCreateFromProgrammableExp(hMetricProgrammable, pParameterValues, parameterCount, pName, pDescription, pMetricHandleCount, phMetricHandles)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetDeviceCreateMetricGroupsFromMetricsExp(hDevice, metricCount, phMetrics, pMetricGroupNamePrefix, pDescription, pMetricGroupCount, phMetricGroup)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupCreateExp(hDevice, pName, pDescription, samplingType, phMetricGroup)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupAddMetricExp(hMetricGroup, hMetric, pErrorStringSize, pErrorString)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupRemoveMetricExp(hMetricGroup, hMetric)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupCloseExp(hMetricGroup)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricGroupDestroyExp(hMetricGroup)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zetMetricDestroyExp(hMetric)");
