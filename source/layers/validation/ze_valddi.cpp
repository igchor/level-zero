...
namespace validation_layer
{
    static ze_result_t logAndPropagateResult(const char* fname, ze_result_t result) {
        ...
        if (logger && result != ZE_RESULT_SUCCESS) {
            logger->log_trace("Error (" + loader::to_string(result) + ") in " + std::string(fname));
        }
        return result;
    }
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDriverGet(pCount, phDrivers)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeInitDrivers(pCount, phDrivers, desc)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDriverGetApiVersion(hDriver, version)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDriverGetProperties(hDriver, pDriverProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDriverGetIpcProperties(hDriver, pIpcProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDriverGetExtensionProperties(hDriver, pCount, pExtensionProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDriverGetExtensionFunctionAddress(hDriver, name, ppFunctionAddress)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDriverGetLastErrorDescription(hDriver, ppString)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGet(hDriver, pCount, phDevices)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetRootDevice(hDevice, phRootDevice)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetSubDevices(hDevice, pCount, phSubdevices)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetProperties(hDevice, pDeviceProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetComputeProperties(hDevice, pComputeProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetCommandQueueGroupProperties(hDevice, pCount, pCommandQueueGroupProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetMemoryProperties(hDevice, pCount, pMemProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetMemoryAccessProperties(hDevice, pMemAccessProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetImageProperties(hDevice, pImageProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetExternalMemoryProperties(hDevice, pExternalMemoryProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetP2PProperties(hDevice, hPeerDevice, pP2PProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceCanAccessPeer(hDevice, hPeerDevice, value)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetStatus(hDevice)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetGlobalTimestamps(hDevice, hostTimestamp, deviceTimestamp)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeContextCreate(hDriver, desc, phContext)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeContextCreateEx(hDriver, desc, numDevices, phDevicesLocal, phContext)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeContextDestroy(hContext)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeContextGetStatus(hContext)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandQueueCreate(hContext, hDevice, desc, phCommandQueue)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandQueueExecuteCommandLists(hCommandQueue, numCommandLists, phCommandListsLocal, hFence)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandQueueSynchronize(hCommandQueue, timeout)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandQueueGetOrdinal(hCommandQueue, pOrdinal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandQueueGetIndex(hCommandQueue, pIndex)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListCreate(hContext, hDevice, desc, phCommandList)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListCreateImmediate(hContext, hDevice, altdesc, phCommandList)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListDestroy(hCommandList)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListClose(hCommandList)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListReset(hCommandList)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendWriteGlobalTimestamp(hCommandList, dstptr, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListHostSynchronize(hCommandList, timeout)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListGetDeviceHandle(hCommandList, phDevice)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListGetContextHandle(hCommandList, phContext)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListGetOrdinal(hCommandList, pOrdinal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListImmediateGetIndex(hCommandListImmediate, pIndex)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListIsImmediate(hCommandList, pIsImmediate)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendBarrier(hCommandList, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendMemoryRangesBarrier(hCommandList, numRanges, pRangeSizes, pRanges, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeContextSystemBarrier(hContext, hDevice)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendMemoryCopy(hCommandList, dstptr, srcptr, size, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendMemoryFill(hCommandList, ptr, pattern, pattern_size, size, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendMemoryCopyRegion(hCommandList, dstptr, dstRegion, dstPitch, dstSlicePitch, srcptr, srcRegion, srcPitch, srcSlicePitch, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendMemoryCopyFromContext(hCommandList, dstptr, hContextSrc, srcptr, size, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendImageCopy(hCommandList, hDstImage, hSrcImage, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendImageCopyRegion(hCommandList, hDstImage, hSrcImage, pDstRegion, pSrcRegion, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendImageCopyToMemory(hCommandList, dstptr, hSrcImage, pSrcRegion, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendImageCopyFromMemory(hCommandList, hDstImage, srcptr, pDstRegion, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendMemoryPrefetch(hCommandList, ptr, size)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendMemAdvise(hCommandList, hDevice, ptr, size, advice)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventPoolCreate(hContext, desc, numDevices, phDevicesLocal, phEventPool)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventPoolDestroy(hEventPool)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventCreate(hEventPool, desc, phEvent)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventDestroy(hEvent)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventPoolPutIpcHandle(hContext, hIpc)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventPoolOpenIpcHandle(hContext, hIpc, phEventPool)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventPoolCloseIpcHandle(hEventPool)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendSignalEvent(hCommandList, hEvent)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendWaitOnEvents(hCommandList, numEvents, phEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventHostSignal(hEvent)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventHostSynchronize(hEvent, timeout)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventQueryStatus(hEvent)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendEventReset(hCommandList, hEvent)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventHostReset(hEvent)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventQueryKernelTimestamp(hEvent, dstptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendQueryKernelTimestamps(hCommandList, numEvents, phEventsLocal, dstptr, pOffsets, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventGetEventPool(hEvent, phEventPool)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventGetSignalScope(hEvent, pSignalScope)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventGetWaitScope(hEvent, pWaitScope)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventPoolGetContextHandle(hEventPool, phContext)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventPoolGetFlags(hEventPool, pFlags)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFenceCreate(hCommandQueue, desc, phFence)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFenceDestroy(hFence)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFenceHostSynchronize(hFence, timeout)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFenceQueryStatus(hFence)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFenceReset(hFence)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeImageGetProperties(hDevice, desc, pImageProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeImageCreate(hContext, hDevice, desc, phImage)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeImageDestroy(hImage)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemAllocShared(hContext, device_desc, host_desc, size, alignment, hDevice, pptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemAllocDevice(hContext, device_desc, size, alignment, hDevice, pptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemAllocHost(hContext, host_desc, size, alignment, pptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemFree(hContext, ptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemGetAllocProperties(hContext, ptr, pMemAllocProperties, phDevice)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemGetAddressRange(hContext, ptr, pBase, pSize)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemGetIpcHandle(hContext, ptr, pIpcHandle)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemGetIpcHandleFromFileDescriptorExp(hContext, handle, pIpcHandle)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemGetFileDescriptorFromIpcHandleExp(hContext, ipcHandle, pHandle)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemPutIpcHandle(hContext, handle)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemOpenIpcHandle(hContext, hDevice, handle, flags, pptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemCloseIpcHandle(hContext, ptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemSetAtomicAccessAttributeExp(hContext, hDevice, ptr, size, attr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemGetAtomicAccessAttributeExp(hContext, hDevice, ptr, size, pAttr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleCreate(hContext, hDevice, desc, phModule, phBuildLog)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleDestroy(hModule)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleDynamicLink(numModules, phModulesLocal, phLinkLog)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleBuildLogDestroy(hModuleBuildLog)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleBuildLogGetString(hModuleBuildLog, pSize, pBuildLog)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleGetNativeBinary(hModule, pSize, pModuleNativeBinary)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleGetGlobalPointer(hModule, pGlobalName, pSize, pptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleGetKernelNames(hModule, pCount, pNames)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleGetProperties(hModule, pModuleProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelCreate(hModule, desc, phKernel)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelDestroy(hKernel)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleGetFunctionPointer(hModule, pFunctionName, pfnFunction)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelSetGroupSize(hKernel, groupSizeX, groupSizeY, groupSizeZ)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelSuggestGroupSize(hKernel, globalSizeX, globalSizeY, globalSizeZ, groupSizeX, groupSizeY, groupSizeZ)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelSuggestMaxCooperativeGroupCount(hKernel, totalGroupCount)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelSetArgumentValue(hKernel, argIndex, argSize, pArgValue)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelSetIndirectAccess(hKernel, flags)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelGetIndirectAccess(hKernel, pFlags)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelGetSourceAttributes(hKernel, pSize, pString)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelSetCacheConfig(hKernel, flags)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelGetProperties(hKernel, pKernelProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelGetName(hKernel, pSize, pName)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendLaunchKernel(hCommandList, hKernel, pLaunchFuncArgs, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendLaunchCooperativeKernel(hCommandList, hKernel, pLaunchFuncArgs, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendLaunchKernelIndirect(hCommandList, hKernel, pLaunchArgumentsBuffer, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendLaunchMultipleKernelsIndirect(hCommandList, numKernels, phKernelsLocal, pCountBuffer, pLaunchArgumentsBuffer, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeContextMakeMemoryResident(hContext, hDevice, ptr, size)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeContextEvictMemory(hContext, hDevice, ptr, size)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeContextMakeImageResident(hContext, hDevice, hImage)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeContextEvictImage(hContext, hDevice, hImage)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeSamplerCreate(hContext, hDevice, desc, phSampler)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeSamplerDestroy(hSampler)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeVirtualMemReserve(hContext, pStart, size, pptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeVirtualMemQueryPageSize(hContext, hDevice, size, pagesize)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zePhysicalMemCreate(hContext, hDevice, desc, phPhysicalMemory)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zePhysicalMemDestroy(hContext, hPhysicalMemory)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeVirtualMemMap(hContext, ptr, size, hPhysicalMemory, offset, access)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeVirtualMemSetAccessAttribute(hContext, ptr, size, access)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeVirtualMemGetAccessAttribute(hContext, ptr, size, access, outSize)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelSetGlobalOffsetExp(hKernel, offsetX, offsetY, offsetZ)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelGetBinaryExp(hKernel, pSize, pKernelBinary)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceImportExternalSemaphoreExt(hDevice, desc, phSemaphore)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceReleaseExternalSemaphoreExt(hSemaphore)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendSignalExternalSemaphoreExt(hCommandList, numSemaphores, phSemaphoresLocal, signalParams, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendWaitExternalSemaphoreExt(hCommandList, numSemaphores, phSemaphoresLocal, waitParams, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceReserveCacheExt(hDevice, cacheLevel, cacheReservationSize)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceSetCacheAdviceExt(hDevice, ptr, regionSize, cacheRegion)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventQueryTimestampsExp(hEvent, hDevice, pCount, pTimestamps)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeImageGetMemoryPropertiesExp(hImage, pMemoryProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeImageViewCreateExt(hContext, hDevice, desc, hImage, phImageView)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeImageViewCreateExp(hContext, hDevice, desc, hImage, phImageView)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeKernelSchedulingHintExp(hKernel, pHint)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDevicePciGetPropertiesExt(hDevice, pPciProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendImageCopyToMemoryExt(hCommandList, dstptr, hSrcImage, pSrcRegion, destRowPitch, destSlicePitch, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListAppendImageCopyFromMemoryExt(hCommandList, hDstImage, srcptr, pDstRegion, srcRowPitch, srcSlicePitch, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeImageGetAllocPropertiesExt(hContext, hImage, pImageAllocProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeModuleInspectLinkageExt(pInspectDesc, numModules, phModulesLocal, phLog)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemFreeExt(hContext, pMemFreeDesc, ptr)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFabricVertexGetExp(hDriver, pCount, phVertices)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFabricVertexGetSubVerticesExp(hVertex, pCount, phSubvertices)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFabricVertexGetPropertiesExp(hVertex, pVertexProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFabricVertexGetDeviceExp(hVertex, phDevice)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDeviceGetFabricVertexExp(hDevice, phVertex)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFabricEdgeGetExp(hVertexA, hVertexB, pCount, phEdges)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFabricEdgeGetVerticesExp(hEdge, phVertexA, phVertexB)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeFabricEdgeGetPropertiesExp(hEdge, pEdgeProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeEventQueryKernelTimestampsExt(hEvent, hDevice, pCount, pResults)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeRTASBuilderCreateExp(hDriver, pDescriptor, phBuilder)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeDriverRTASFormatCompatibilityCheckExp(hDriver, rtasFormatA, rtasFormatB)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeRTASBuilderBuildExp(hBuilder, pBuildOpDescriptor, pScratchBuffer, scratchBufferSizeBytes, pRtasBuffer, rtasBufferSizeBytes, hParallelOperation, pBuildUserPtr, pBounds, pRtasBufferSizeBytes)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeRTASBuilderDestroyExp(hBuilder)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeRTASParallelOperationCreateExp(hDriver, phParallelOperation)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeRTASParallelOperationGetPropertiesExp(hParallelOperation, pProperties)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeRTASParallelOperationJoinExp(hParallelOperation)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeRTASParallelOperationDestroyExp(hParallelOperation)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeMemGetPitchFor2dImage(hContext, hDevice, imageWidth, imageHeight, elementSizeInBytes, rowPitch)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeImageGetDeviceOffsetExp(hImage, pDeviceOffset)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListCreateCloneExp(hCommandList, phClonedCommandList)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListImmediateAppendCommandListsExp(hCommandListImmediate, numCommandLists, phCommandListsLocal, hSignalEvent, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListGetNextCommandIdExp(hCommandList, desc, pCommandId)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListGetNextCommandIdWithKernelsExp(hCommandList, desc, numKernels, phKernelsLocal, pCommandId)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListUpdateMutableCommandsExp(hCommandList, desc)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListUpdateMutableCommandSignalEventExp(hCommandList, commandId, hSignalEvent)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListUpdateMutableCommandWaitEventsExp(hCommandList, commandId, numWaitEvents, phWaitEventsLocal)");
        ...
        )
    {
        auto logger = context.logger;
        if (logger)
            logger->log_trace("zeCommandListUpdateMutableCommandKernelsExp(hCommandList, numKernels, pCommandId, phKernelsLocal)");
